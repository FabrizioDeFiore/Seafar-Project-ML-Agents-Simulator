...
public class VasselDriver : MonoBehaviour {

    private const int SOLID_OBJECTS_LAYER = -1;

    private float speed;
    private float speedMax = 300f; //70
    private float speedMin = -50f;
    private float acceleration = 150f;  //30
    ...
    private Rigidbody vasselRigidbody;

    private void Awake() {
        vasselRigidbody = GetComponent<Rigidbody>();
    }
    ...
    private void FixedUpdate() {
        ...
        vasselRigidbody.velocity = transform.forward * speed;
        ...
        vasselRigidbody.angularVelocity = new Vector3(0, turnSpeed * (invertSpeedNormalized * 1f) * Mathf.Deg2Rad, 0);
...
*/
    ...
    public Vector3 GetRigidbodyVelocity() {
        return vasselRigidbody.velocity;
    }
